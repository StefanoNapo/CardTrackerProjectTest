...
class CardRepository @Inject constructor(private val api : CardService, private val cardDao: CardDao) {
    ...
    suspend fun searchCardsWithTypeMonTypeAtkDef (searchQuery: String, searchType: String, monsType: String, searchAtk: Int, searchDef: Int): List<Card>{
        val response = cardDao.searchCardsWithTypeMonTypeAtkDef(searchQuery, searchType, monsType, searchAtk, searchDef)
        return response.map { it.toDomain() }
    }

    suspend fun searchCardsWithTypeMonTypeAtkLvl (searchQuery: String, searchType: String, monsType: String, searchAtk: Int, searchLvl: Int): List<Card>{
        val response = cardDao.searchCardsWithTypeMonTypeAtkLvl(searchQuery, searchType, monsType, searchAtk, searchLvl)
        return response.map { it.toDomain() }
    }
    ...
    suspend fun searchCardsWithTAMTAtkL (searchQuery: String, searchType: String, searchAttr: String, monsType: String, searchAtk: Int, searchLvl: Int): List<Card>{
        val response = cardDao.searchCardsWithTAMTAtkL(searchQuery, searchType, searchAttr, monsType, searchAtk, searchLvl)
        return response.map { it.toDomain() }
    }

    suspend fun searchCardsWithTAMTDL (searchQuery: String, searchType: String, searchAttr: String, monsType: String, searchDef: Int, searchLvl: Int): List<Card>{
        val response = cardDao.searchCardsWithTAMTDL(searchQuery, searchType, searchAttr, monsType, searchDef, searchLvl)
        return response.map { it.toDomain() }
    }

    suspend fun searchCardsWithTAAtkDL (searchQuery: String, searchType: String, searchAttr: String, searchAtk: Int, searchDef: Int, searchLvl: Int): List<Card>{
        val response = cardDao.searchCardsWithTAAtkDL(searchQuery, searchType, searchAttr, searchAtk, searchDef, searchLvl)
        return response.map { it.toDomain() }
    }

    suspend fun searchCardsWithTMTAtkDL (searchQuery: String, searchType: String, monsType: String, searchAtk: Int, searchDef: Int, searchLvl: Int): List<Card>{
        val response = cardDao.searchCardsWithTMTAtkDL(searchQuery, searchType, monsType, searchAtk, searchDef, searchLvl)
        return response.map { it.toDomain() }
    }

    suspend fun searchCardsWithMTAAtkDL (searchQuery: String, monsType: String, searchAttr: String, searchAtk: Int, searchDef: Int, searchLvl: Int): List<Card>{
        val response = cardDao.searchCardsWithMTAAtkDL(searchQuery, monsType, searchAttr, searchAtk, searchDef, searchLvl)
        return response.map { it.toDomain() }
    }
